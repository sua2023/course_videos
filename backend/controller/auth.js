...
const authLogin = (req, res) => {
    ...
    if (!username && !email) {
        return res.status(400).json({ status: 400, message: "Username or Email required" })
    }
    ...
    conn.query('SELECT id, username,email, password FROM users WHERE username = ? or email=?', [username, email], (err, results) => {
        ...
        bcrypt.compare(password, newPassword, (err, isMatch) => {
            ...
            const token = jwt.sign({ id: user.id, username: user.username, email: user.email }, secrekey, { expiresIn: '24h' })
